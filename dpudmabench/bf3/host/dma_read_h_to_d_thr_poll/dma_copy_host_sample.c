...
dma_copy_dpu(char *export_desc_file_path, char *buffer_info_file_path, const char *pcie_addr)
{
	...
	for (int i = 0; i < N; i++) {
// read
		// result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, resources.remote_mmap, resources.remote_addr, resources.remote_addr_len, &(resources.src_doca_buf_array[i]));
// write
		result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, resources.remote_mmap, resources.remote_addr, resources.remote_addr_len, &(resources.dst_doca_buf_array[i]));
		if (result != DOCA_SUCCESS) {
...
// read
		// result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, state->dst_mmap, resources.dpu_buffer, resources.dst_buffer_size, &(resources.dst_doca_buf_array[i]));
// write
		result = doca_buf_inventory_buf_get_by_addr(state->buf_inv, state->dst_mmap, resources.dpu_buffer, resources.dst_buffer_size, &(resources.src_doca_buf_array[i]));
		if (result != DOCA_SUCCESS) {
...
// read
		// result = doca_buf_set_data(resources.src_doca_buf_array[i], resources.remote_addr, resources.remote_addr_len);
// write
		result = doca_buf_set_data(resources.src_doca_buf_array[i], resources.dpu_buffer, resources.dst_buffer_size);
		if (result != DOCA_SUCCESS) {
...
// write
	...
	resources.num_remaining_tasks = N * repeat_count;
	...
	// }
	for (int i = 0; i < N; i++) {
		result = doca_dma_task_memcpy_alloc_init(resources.dma_ctx, resources.src_doca_buf_array[i], resources.dst_doca_buf_array[i], task_user_data, &(resources.tasks[i]));
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to allocate DMA memcpy task: %s", doca_error_get_descr(result));
			goto destroy_dst_buf;
		}
	...
	for (int i = 0; i < repeat_count; i++) {
		for (int j = 0; j < inner_loop; j++) {
			result = doca_task_submit(doca_dma_task_memcpy_as_task(resources.tasks[j]));
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed to submit DMA task: %s", doca_error_get_descr(result));
				goto destroy_dst_buf;
			}
		} 
		// printf("Iteration %d submit done\n", i);

		while (resources.num_remaining_tasks != ((repeat_count-i) * inner_loop - inner_loop)) {
			(void)doca_pe_progress(state->pe);
			// printf("num_remaining_tasks: %d\n", resources.num_remaining_tasks);
		}
		num_completions += inner_loop;
		// printf("Iteration %d done\n", i);
	}

	// for (int i = 0; i < N; i++) {
	// 		// clock_gettime(CLOCK_REALTIME, &(resources->blk_time_end[N-resources->num_remaining_tasks]));
	// 	clock_gettime(CLOCK_REALTIME, &(resources.blk_time_start[i]));
	// 	result = doca_task_submit(doca_dma_task_memcpy_as_task(resources.tasks[i]));
	// 	if (result != DOCA_SUCCESS) {
	// 		DOCA_LOG_ERR("Failed to submit DMA task: %s", doca_error_get_descr(result));
	// 		goto destroy_dst_buf;
	// 	}

	// 	while(doca_pe_progress(state->pe) != 1) {
	// 		// if (resources.num_remaining_tasks == 0) {
	// 		// 	break;
	// 		// }
	// 		// sleep(1);
	// 	}
	// 	clock_gettime(CLOCK_REALTIME, &(resources.blk_time_end[i]));
	// 	// printf("Task %d done\n", i);
	// }
	...
	double thr = ((double)num_completions / blk_ns) * 1000000000;
	thr = thr / 1000000.0;
	printf("Average blocking DMA (D-to-H) (polling) throughput (Mops) = %.3f\n", thr);
...
free_task:
	for (int i = 0; i < N; i++) {
		doca_task_free(doca_dma_task_memcpy_as_task(resources.tasks[i]));
	}
destroy_dst_buf:
	// tmp_result = doca_buf_dec_refcount(resources.dst_doca_buf, NULL);
	for (int i = 0; i < N; i++) {
		tmp_result = doca_buf_dec_refcount(resources.dst_doca_buf_array[i], NULL);
...
destroy_src_buf:
	...
	for (int i = 0; i < N; i++) {
		tmp_result = doca_buf_dec_refcount(resources.src_doca_buf_array[i], NULL);
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_ERROR_PROPAGATE(result, tmp_result);
