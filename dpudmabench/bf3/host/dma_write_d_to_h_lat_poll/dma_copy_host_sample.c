...
//write
	result = doca_mmap_set_permissions(state.src_mmap, DOCA_ACCESS_FLAG_PCI_READ_WRITE);
	if (result != DOCA_SUCCESS) {
	...
	// char *test_buffer;
	// test_buffer = (char *)malloc(src_buffer_size);

	// result = doca_mmap_set_memrange(state.src_mmap, test_buffer, src_buffer_size);
	// if (result != DOCA_SUCCESS) {
	...
	// 	goto destroy_resources;
	// }
	// printf("doca_mmap_start success!\n");
