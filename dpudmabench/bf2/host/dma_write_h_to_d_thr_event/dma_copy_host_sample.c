...
dma_copy_from_dpu(char *export_desc_file_path, char *buffer_info_file_path, struct doca_pci_bdf *pcie_addr)
{
	...
	if (result != DOCA_SUCCESS) {
        ...
        // Blocking DMA write throughput test
	...
	clock_gettime(CLOCK_REALTIME, &start);
		...
		for (j = 0; j < 1024; j++) {
			...
			// Even though we spin for small buffer sizes it enters only once. This is useful for large buffer sizes as event is not 
			// sufficient to check for compleition.
	...
	clock_gettime(CLOCK_REALTIME, &end);
        ...
        // Converting to Kops
        thr = thr / 1000.0;
        printf("Average blocking dma write (H-to-D) throughput (Kops) = %.2f\n", thr);
