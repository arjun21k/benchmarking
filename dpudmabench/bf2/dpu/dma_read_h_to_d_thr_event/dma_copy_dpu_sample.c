 ...
 */
doca_error_t
dma_copy_to_host(struct doca_pci_bdf *pcie_addr, char *src_buffer, size_t src_buffer_size,
		     char *export_desc_file_path, char *buffer_info_file_name)
{
	...
	DOCA_LOG_INFO("Please copy %s and %s to the HOST and run DMA Copy HOST sample before closing", export_desc_file_path, buffer_info_file_name);
