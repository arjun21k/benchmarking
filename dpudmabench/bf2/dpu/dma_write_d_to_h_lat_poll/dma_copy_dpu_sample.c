...
dma_copy_dpu(char *export_desc_file_path, char *buffer_info_file_path, struct doca_pci_bdf *pcie_addr)
{
	...
	int r = posix_memalign((void**)&dpu_buffer, 64, dst_buffer_size);
	//if (dpu_buffer == NULL) {
	if ( r!= 0) {
	...
	memset(dpu_buffer, 0, dst_buffer_size);
	...
	long N = 500000;
        double blk_time[N]; // non_blk_time[5000];
        int i;
        for ( i = 0; i < N; i++) {
		...
		//clock_gettime(CLOCK_REALTIME, &non_blk_end);
		...
		//non_blk_ns = (non_blk_end.tv_sec - start.tv_sec)*1000000000 + (non_blk_end.tv_nsec - start.tv_nsec);
		blk_time[i] = blk_ns;
		//non_blk_time[i] = non_blk_ns;
        ...
        double std_dev = 0;
        for (i = 0; i < N; i++) {
                std_dev += pow(abs(blk_time[i]-mean_t),2);
        }
        std_dev = sqrt(std_dev/N);
	printf("Blocking DMA write (D-to-H)\n");
	...
	printf("%.2f\t %13.2f\t %13.2f\t %13.2f\n", min_t/1000, mean_t/1000, max_t/1000, std_dev/1000);
        ...
        /*min_t = non_blk_time[0];
        max_t = non_blk_time[0];
	total_time_in_ns = 0;
        for (i = 0; i < 5000; i++) {
                if (non_blk_time[i] <= min_t)
                        min_t = non_blk_time[i];
                if (non_blk_time[i] >= max_t)
                        max_t = non_blk_time[i];
                total_time_in_ns += non_blk_time[i];
        }
        mean_t = (double)total_time_in_ns/5000;
        std_dev = 0;
        for (i = 0; i < 5000; i++) {
                std_dev += pow(abs(blk_time[i]-mean_t),2);
        }
        std_dev = sqrt(std_dev/5000);
